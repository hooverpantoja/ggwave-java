...
#include "test-cpp.h"
#include <C:\Users\camil\AppData\Local\Android\Sdk\ndk\26.1.10909125\toolchains\llvm\prebuilt\windows-x86_64\sysroot\usr\include\android\log.h>
#include <C:\Users\camil\AppData\Local\Android\Sdk\ndk\26.1.10909125\toolchains\llvm\prebuilt\windows-x86_64\sysroot\usr\include\jni.h>
#include "\ggwave\include\ggwave\ggwave.h"
#include <C:\Users\camil\AppData\Local\Android\Sdk\ndk\26.1.10909125\toolchains\llvm\prebuilt\windows-x86_64\sysroot\usr\include\c++\v1\cstring>
...
extern "C"{
    ...
    Java_com_example_ggwave_MainActivity_sendMessage(JNIEnv *env, jobject thiz, jstring message) {
        __android_log_print(ANDROID_LOG_DEBUG, "ggwave (native)", "Encoding message: '%s'", env->GetStringUTFChars(message,
                                                                                                                   nullptr));

        const int n = ggwave_encode(g_ggwave, env->GetStringUTFChars(message, nullptr), env->GetStringLength(message), GGWAVE_TX_PROTOCOL_AUDIBLE_FAST, 10,
                                    nullptr, 1);

        char waveform[n];

        const int ret = ggwave_encode(g_ggwave, env->GetStringUTFChars(message, nullptr), env->GetStringLength(message), GGWAVE_TX_PROTOCOL_AUDIBLE_FAST, 10, waveform, 0);
